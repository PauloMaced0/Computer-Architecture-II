...
int main(void)
{
    ...
    while (1)
    {
        ...
        i = (i + 1) % 50; // increment counter at 2Hz
        if (i == 0 && (PORTB & 0x0001) == 1) counter++;
        else if (i == 0  && (PORTB & 0x0000) == 0)counter--;
        else if (counter > 59 && (PORTB & 0x0001) == 1) counter = 0;
        else if (counter < 0 && (PORTB & 0x0000) == 0) counter = 59;
    }
    return 0;
}
